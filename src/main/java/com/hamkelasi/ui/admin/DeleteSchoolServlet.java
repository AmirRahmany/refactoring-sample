package com.hamkelasi.ui.admin;

import com.hamkelasi.bll.School;
import com.hamkelasi.bll.User;
import jakarta.servlet.RequestDispatcher;
import jakarta.servlet.ServletException;
import jakarta.servlet.annotation.WebServlet;
import jakarta.servlet.http.*;

import java.io.*;

@WebServlet("/admin/delete-schools")
public class DeleteSchoolServlet extends HttpServlet {

    protected void doGet(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        HttpSession session = request.getSession();
        Cookie[] cookies = request.getCookies();

        // Check authentication from cookies
        if (cookies != null) {
            for (Cookie cookie : cookies) {
                if ("UserID".equals(cookie.getName()) && session.getAttribute("UserID") == null) {
                    session.setAttribute("UserID", cookie.getValue());
                }
            }
        }

        boolean isAuthenticated = false;
        String errorMessage = null;

        if (session.getAttribute("UserID") != null) {
            try {
                int loggedID = Integer.parseInt(session.getAttribute("UserID").toString());
                User loggedUser = new User(loggedID);

                if (loggedUser.getPermission() == 1) {
                    isAuthenticated = true;

                    String schoolId = request.getParameter("ID");
                    if (schoolId != null && !schoolId.trim().isEmpty()) {
                        try {
                            int schoolID = Integer.parseInt(schoolId);
                            School school = new School(schoolID);

                            request.setAttribute("schoolId", schoolID);
                            request.setAttribute("schoolName", school.getName());
                        } catch (NumberFormatException e) {
                            response.sendRedirect(request.getContextPath() + "/admin/edit-schools");
                            return;
                        }
                    } else {
                        response.sendRedirect(request.getContextPath() + "/admin/edit-Schools");
                        return;
                    }
                }
            } catch (Exception e) {
                errorMessage = "Error processing request";
            }
        }

        if (!isAuthenticated) {
            errorMessage = "You don't have permission to access this page";
        }

        request.setAttribute("isAuthenticated", isAuthenticated);
        request.setAttribute("errorMessage", errorMessage);

        RequestDispatcher dispatcher = request.getRequestDispatcher("/admin/delete-school.html");
        dispatcher.forward(request, response);
    }

    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {

        String schoolId = request.getParameter("schoolId");
        String action = request.getParameter("action");

        if ("yes".equals(action) && schoolId != null && !schoolId.trim().isEmpty()) {
            try {
                int schoolID = Integer.parseInt(schoolId);
                School school = new School();
                int result = school.delete(schoolID);

                if (result == 0) {
                    response.sendRedirect(request.getContextPath() + "/admin/edit-schools?msg=ok");
                } else if (result == 9) {
                    response.sendRedirect(request.getContextPath() + "/admin/edit-schools?msg=fail");
                }
            } catch (NumberFormatException e) {
                response.sendRedirect(request.getContextPath() + "/admin/edit-schools?msg=fail");
            }
        } else {
            response.sendRedirect(request.getContextPath() + "/admin/edit-schools");
        }
    }
}
